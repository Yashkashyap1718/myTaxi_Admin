...
class PaymentController extends GetxController {
  ...
  Rx<TextEditingController> paypalClientKeyController =
      TextEditingController().obs;
  Rx<TextEditingController> paypalSecretKeyController =
      TextEditingController().obs;
  Rx<Status> isPaypalActive = Status.active.obs;
  Rx<Status> isPaypalSandBox = Status.active.obs;

  // payStack
  Rx<TextEditingController> payStackNameController =
      TextEditingController().obs;
  Rx<TextEditingController> payStackSecretKeyController =
      TextEditingController().obs;
  Rx<Status> isPayStackActive = Status.active.obs;

  // razorpay
  Rx<TextEditingController> razorpayNameController =
      TextEditingController().obs;
  Rx<TextEditingController> razorpayKeyController = TextEditingController().obs;
  Rx<TextEditingController> razorpaySecretController =
      TextEditingController().obs;
  ...
  Rx<TextEditingController> clientPublishableKeyController =
      TextEditingController().obs;
  Rx<TextEditingController> stripeSecretKeyController =
      TextEditingController().obs;
  Rx<Status> isStripeActive = Status.active.obs;
  Rx<Status> isStripeSandBox = Status.active.obs;

  // mercadoPogo
  Rx<TextEditingController> mercadoPogoNameController =
      TextEditingController().obs;
  Rx<TextEditingController> mercadoPogoAccessTokenController =
      TextEditingController().obs;
  Rx<Status> isMercadoPogoActive = Status.active.obs;

  // payFast
  Rx<TextEditingController> payFastNameController = TextEditingController().obs;
  Rx<TextEditingController> payFastMerchantKeyController =
      TextEditingController().obs;
  Rx<TextEditingController> payFastMerchantIDController =
      TextEditingController().obs;
  Rx<TextEditingController> payFastReturnUrlController =
      TextEditingController().obs;
  Rx<TextEditingController> payFastNotifyUrlController =
      TextEditingController().obs;
  Rx<TextEditingController> payFastCancelUrlController =
      TextEditingController().obs;
  Rx<Status> isPayFastSandBox = Status.active.obs;
  Rx<Status> isPayFastActive = Status.active.obs;

  // flutterWave
  Rx<TextEditingController> flutterWaveNameController =
      TextEditingController().obs;
  Rx<TextEditingController> flutterWavePublicKeyKeyController =
      TextEditingController().obs;
  ...
  savePayment() async {
    ...
    // Stripe
      ...
      return ShowToast.errorToast(
          "Please Add Stripe Client Publishable Key".tr);
    ...
    // If all conditions are met, save the payment details
      ...
      paymentModel.value.paypal?.paypalClient =
          paypalClientKeyController.value.text;
      paymentModel.value.paypal?.paypalSecret =
          paypalSecretKeyController.value.text;
      paymentModel.value.paypal?.isActive =
          isPaypalActive.value == Status.active ? true : false;
      paymentModel.value.paypal?.isSandbox =
          isPaypalSandBox.value == Status.active ? true : false;

      // PayStack
      paymentModel.value.payStack?.name = payStackNameController.value.text;
      paymentModel.value.payStack?.payStackSecret =
          payStackSecretKeyController.value.text;
      paymentModel.value.payStack?.isActive =
          isPayStackActive.value == Status.active ? true : false;

      // Razorpay
      paymentModel.value.razorpay?.name = razorpayNameController.value.text;
      paymentModel.value.razorpay?.razorpayKey =
          razorpayKeyController.value.text;
      paymentModel.value.razorpay?.razorpaySecret =
          razorpaySecretController.value.text;
      paymentModel.value.razorpay?.isActive =
          isRazorpayActive.value == Status.active ? true : false;
      paymentModel.value.razorpay?.isSandbox =
          isRazorPaySandBox.value == Status.active ? true : false;

      // Stripe
      paymentModel.value.strip?.name = stripeNameController.value.text;
      paymentModel.value.strip?.clientPublishableKey =
          clientPublishableKeyController.value.text;
      paymentModel.value.strip?.stripeSecret =
          stripeSecretKeyController.value.text;
      paymentModel.value.strip?.isActive =
          isStripeActive.value == Status.active ? true : false;
      paymentModel.value.strip?.isSandbox =
          isStripeSandBox.value == Status.active ? true : false;

      // MercadoPago
      paymentModel.value.mercadoPago?.name =
          mercadoPogoNameController.value.text;
      paymentModel.value.mercadoPago?.mercadoPagoAccessToken =
          mercadoPogoAccessTokenController.value.text;
      paymentModel.value.mercadoPago?.isActive =
          isMercadoPogoActive.value == Status.active ? true : false;

      // PayFast
      paymentModel.value.payFast?.name = payFastNameController.value.text;
      paymentModel.value.payFast?.merchantKey =
          payFastMerchantKeyController.value.text;
      paymentModel.value.payFast?.merchantId =
          payFastMerchantIDController.value.text;
      paymentModel.value.payFast?.returnUrl =
          payFastReturnUrlController.value.text;
      paymentModel.value.payFast?.notifyUrl =
          payFastNotifyUrlController.value.text;
      paymentModel.value.payFast?.cancelUrl =
          payFastCancelUrlController.value.text;
      paymentModel.value.payFast?.isActive =
          isPayFastActive.value == Status.active ? true : false;
      paymentModel.value.payFast?.isSandbox =
          isPayFastSandBox.value == Status.active ? true : false;

      // FlutterWave
      paymentModel.value.flutterWave?.name =
          flutterWaveNameController.value.text;
      paymentModel.value.flutterWave?.publicKey =
          flutterWavePublicKeyKeyController.value.text;
      paymentModel.value.flutterWave?.isActive =
          isFlutterWaveActive.value == Status.active ? true : false;
      paymentModel.value.flutterWave?.isSandBox =
          isFlutterWaveSandBox.value == Status.active ? true : false;

      // Cash
      paymentModel.value.cash?.name = cashNameController.value.text;
      paymentModel.value.cash?.isActive =
          isCashActive.value == Status.active ? true : false;

      // Wallet
      paymentModel.value.wallet?.name = walletNameController.value.text;
      paymentModel.value.wallet?.isActive =
          isWalletActive.value == Status.active ? true : false;

      // Print details step by step
      print(
          'PayPal: Name=${paymentModel.value.paypal?.name}, Client=${paymentModel.value.paypal?.paypalClient}, Secret=${paymentModel.value.paypal?.paypalSecret}, Active=${paymentModel.value.paypal?.isActive}, Sandbox=${paymentModel.value.paypal?.isSandbox}');
      print(
          'PayStack: Name=${paymentModel.value.payStack?.name}, Secret=${paymentModel.value.payStack?.payStackSecret}, Active=${paymentModel.value.payStack?.isActive}');
      print(
          'Razorpay: Name=${paymentModel.value.razorpay?.name}, Key=${paymentModel.value.razorpay?.razorpayKey}, Secret=${paymentModel.value.razorpay?.razorpaySecret}, Active=${paymentModel.value.razorpay?.isActive}, Sandbox=${paymentModel.value.razorpay?.isSandbox}');
      print(
          'Stripe: Name=${paymentModel.value.strip?.name}, ClientPublishableKey=${paymentModel.value.strip?.clientPublishableKey}, Secret=${paymentModel.value.strip?.stripeSecret}, Active=${paymentModel.value.strip?.isActive}, Sandbox=${paymentModel.value.strip?.isSandbox}');
      print(
          'MercadoPago: Name=${paymentModel.value.mercadoPago?.name}, AccessToken=${paymentModel.value.mercadoPago?.mercadoPagoAccessToken}, Active=${paymentModel.value.mercadoPago?.isActive}');
      print(
          'PayFast: Name=${paymentModel.value.payFast?.name}, MerchantKey=${paymentModel.value.payFast?.merchantKey}, MerchantID=${paymentModel.value.payFast?.merchantId}, ReturnUrl=${paymentModel.value.payFast?.returnUrl}, NotifyUrl=${paymentModel.value.payFast?.notifyUrl}, CancelUrl=${paymentModel.value.payFast?.cancelUrl}, Active=${paymentModel.value.payFast?.isActive}, Sandbox=${paymentModel.value.payFast?.isSandbox}');
      print(
          'FlutterWave: Name=${paymentModel.value.flutterWave?.name}, PublicKey=${paymentModel.value.flutterWave?.publicKey}, Active=${paymentModel.value.flutterWave?.isActive}, Sandbox=${paymentModel.value.flutterWave?.isSandBox}');
      print(
          'Cash: Name=${paymentModel.value.cash?.name},cash active ${paymentModel.value.cash?.isActive} ');
      print(
          'Wallet : Name=${paymentModel.value.wallet?.name},cash active ${paymentModel.value.wallet?.isActive} ');
